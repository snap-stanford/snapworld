...
class HTTPRequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if subpath[1] == "acquire":
            ...
            logging.debug("acquire %d %s %s" % (len(s), tpid, result))
            return
            ...
            logging.debug("release %d %s" % (len(s), tpid))
            return
...
if __name__ == '__main__':
    ...
    # Set up logging
    logging.basicConfig(filename=fname, level=logging.INFO, format='[%(asctime)s] [%(levelname)s] [%(process)d] [%(filename)s] [%(funcName)s] %(message)s')
